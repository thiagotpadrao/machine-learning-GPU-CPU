...
double* calculaDistancia(double **xtrain,  double **xtest, int linhaAtual){
    ...
    #pragma omp parallel for private(d)     //paralelização dos cálculos para cada linha de treino
...
double* knn (double **xtrain, double *ytrain, double **xtest){
    ...
    #pragma omp parallel for        //paralelização do loop principal do knn
...
int main (int argc, char *argv[]) {
    ...
    omp_set_num_threads(512);       //definição do número de threads utilizadas
